...
def solveA(antennas_map, map_size):
    ...
    for value, positions in antennas_map.items():
        for i, pos1 in enumerate(positions):
            for pos2 in positions[i + 1:]:
                ...
                antinode1 = (
                    pos1[0] + np.sign(pos1[0] - pos2[0]) * dist[0], pos1[1] + np.sign(pos1[1] - pos2[1]) * dist[1])
                antinode2 = (
                    pos2[0] - np.sign(pos1[0] - pos2[0]) * dist[0], pos2[1] - np.sign(pos1[1] - pos2[1]) * dist[1])
...
def main():
    with open('input.txt', 'r') as file:
        ...
        # task 1
        pos = solveA(antennas_positions, map_size)
        print_map(lines, pos)
